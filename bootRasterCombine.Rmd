...
BCRList <- c("9", "10", "11") #, "12", "4", "83", "82", "81", "80", "61", "71", "70", "14", "60")
...
x <- as.list(BCRRasters)
names(x)[1:2] <- c('x', 'y')
x$fun <- mean
x$na.rm <- TRUE
y <- do.call(mosaic, x)
names(y)[1:2] <- c(paste0('mean', birdSp), paste0('variance', birdSp))

#save mean raster
raster::writeRaster(y$paste0('mean', birdSp), filename = paste0(fileCombinedRasters,"/mean", birdSp,".tif"), format = 'GTiff')

#save variance raster
raster::writeRaster(y$paste0('variance', birdSp),filename = paste0(fileCombinedRasters,"/var", birdSp,".tif"), format = 'GTiff')

#delete raw boot files from computer
